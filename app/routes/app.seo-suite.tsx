...
export const action=async({request}:ActionFunctionArgs)=>{
 ...
 if(['suite-export','suite-organic-export'].includes(input.intent)){const row=await ensureWorkspace(db,session.shop);return {ok:true,download:(input.intent==='suite-organic-export'?organicCSV:reportCSV)(suiteData(JSON.parse(row.data))),filename:input.intent==='suite-organic-export'?'nowusa-organik-siralamalar.csv':'nowusa-seo-merkezi.csv'};}
