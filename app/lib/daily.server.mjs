...
export async function runDaily(request, db, run = performOperation) {
  ...
  if (task && !['google', 'backlinks', 'suite'].includes(task)) return new Response('Unknown task', { status: 400, headers });
  ...
  for (const intent of intents) {
    try { const result=await run({ db, shop, admin, input: { intent } });results[intent]=result.ok===false?'failed':result.message; }
