...
function client(){if(!googleConfigured())throw new Error('Google bağlantısı üçün serverdə OAuth parametrləri qurulmalıdır.');return new OAuth2Client(process.env.GOOGLE_CLIENT_ID,process.env.GOOGLE_CLIENT_SECRET,`${appOrigin()}/google/callback`);}
...
export async function startGoogle(db,ticket){
 if(!/^[a-f0-9]{64}$/.test(ticket||''))throw new Error('Bağlantı linki düzgün deyil.');
 const row=await db.googleOAuth.findUnique({where:{ticketHash:hash(ticket)}});if(!row||row.expiresAt<new Date())throw new Error('Bağlantı linkinin vaxtı bitib.');
 ...
 const consumed=await db.googleOAuth.updateMany({where:{id:row.id,ticketHash:hash(ticket),expiresAt:{gt:new Date()}},data:{ticketHash:null,stateHash:hash(state),verifier:seal(codeVerifier,row.shop)}});if(consumed.count!==1)throw new Error('Bağlantı linki artıq istifadə edilib.');
...
export async function finishGoogle(db,{state,code,cookie,error},oauthFactory=client){
 ...
 if(!/^[a-f0-9]{64}$/.test(state||'')||!equal(state,browserState))throw new Error('Google bağlantısı başladılan brauzerdə tamamlanmalıdır.');
 const row=await db.googleOAuth.findUnique({where:{stateHash:hash(state)}});if(!row||row.expiresAt<new Date())throw new Error('Google bağlantısının vaxtı bitib. Yenidən başlayın.');
 const consumed=await db.googleOAuth.deleteMany({where:{id:row.id,stateHash:hash(state)}});if(consumed.count!==1)throw new Error('Google cavabı artıq istifadə edilib.');
 if(error||!code)throw new Error('Google icazəsi verilmədi. Yenidən qoşula bilərsiniz.');
 const oauth=oauthFactory();let tokens;
 try{({tokens}=await oauth.getToken({code,codeVerifier:unseal(row.verifier,row.shop)}));}catch{throw new Error('Google giriş kodu qəbul edilmədi. Yenidən qoşulun.');}
 if(!tokens.refresh_token||!tokens.scope?.split(' ').includes(scope))throw new Error('Google oxuma və offline icazəsi alınmadı. Yenidən qoşulun.');
 oauth.setCredentials(tokens);let properties;
 try{const r=await oauth.request({url:'https://www.googleapis.com/webmasters/v3/sites',timeout:15000});properties=(r.data.siteEntry||[]).filter(s=>s.permissionLevel!=='siteUnverifiedUser'&&ownProperty(s.siteUrl)).map(s=>s.siteUrl);}catch{throw new Error('Search Console saytları alınmadı. Google Cloud-da API-ni aktivləşdirin.');}
 if(!properties.length)throw new Error('Bu Google hesabında nosweatusa.com Search Console property-si tapılmadı.');
 await db.$transaction(async tx=>{if(!await tx.seoWorkspace.findUnique({where:{shop:row.shop}}))throw new Error('Shopify tətbiqi artıq bağlı deyil.');await tx.googleConnection.upsert({where:{shop:row.shop},create:{shop:row.shop,tokens:seal(tokens,row.shop),properties:JSON.stringify(properties),property:properties[0]},update:{tokens:seal(tokens,row.shop),properties:JSON.stringify(properties),property:properties[0],syncedAt:null}});});
 return row.shop;
}
...
export async function fetchGoogleReport(oauth,property,startDate,endDate,{dimensions=['page'],country=null,maxRows=50000}={}){
 ...
 for(let startRow=0;startRow<maxRows;startRow+=rowLimit){
  ...
  for(const row of batch){
   if(!Array.isArray(row.keys)||row.keys.length!==dimensions.length||![row.clicks,row.impressions,row.position].every(n=>Number.isFinite(n)&&n>=0)||row.clicks>row.impressions)throw new Error('Google hesabatının formatı düzgün deyil.');
...
export async function syncGoogle(db,shop,state,oauthFactory=client){
 const row=await db.googleConnection.findUnique({where:{shop}});if(!row?.property||!ownProperty(row.property))throw new Error('Əvvəlcə Google hesabını qoşun.');
 ...
 }catch{throw new Error('Google hesabatı alınmadı. Hesab icazəsini və API bağlantısını yoxlayın; lazım olsa yenidən qoşulun.');}
 const saved=await db.googleConnection.updateMany({where:{shop,tokens:row.tokens},data:{tokens:seal(oauth.credentials,shop),syncedAt:new Date()}});if(!saved.count)throw new Error('Google bağlantısı dəyişib; əməliyyatı təkrarlayın.');
 ...
 return [...reports,...detailed].some(r=>r.truncated)?'ABŞ hesabatları gətirildi. Bəzi hesabatlarda sətir limiti dolub; paneldə məhdudiyyət göstərilir.':'ABŞ üzrə məhsul, açar söz və gündəlik Google hesabatları gətirildi.';
}
...
export async function disconnectGoogle(db,shop){
 ...
 return revoked?'Google bağlantısı ayrıldı.':'Yerli Google bağlantısı silindi. Google hesabının üçüncü tərəf girişləri bölməsində icazəni də ləğv edin.';
}
