...
export const manualChecks=[
 {key:'shipping',label:'ABŞ-a çatdırılma qiyməti və müddəti aydın göstərilir'},
 {key:'returns',label:'Qaytarma şərtləri alıcıya görünür'},
 {key:'facts',label:'Məhsulun xüsusiyyətləri və iddiaları yoxlanılıb'},
 {key:'reviews',label:'Mövcud rəylər realdır; saxta rəy və reytinq yoxdur'},
 {key:'mobile',label:'Mobil telefonda məhsul seçimi və alış axını yoxlanılıb'},
];
...
export function readiness(page,state){const saved=state.growth?.checks?.[page.id]||{};return [
 {key:'photos',label:'Kataloqda məhsul şəkli var',status:page.images?.length?'yes':'no',automatic:true,updatedAt:null},
 {key:'description',label:'Kataloqda məhsul təsviri var',status:plain(page.descriptionHtml)?'yes':'no',automatic:true,updatedAt:null},
 ...manualChecks.map(c=>({...c,status:saved[c.key]?.status||'unknown',automatic:false,updatedAt:saved[c.key]?.updatedAt||null})),
 ];}
export function setReadiness(state,{pageId,check,status}){if(!state.catalog.pages.some(p=>p.id===pageId&&p.type==='product')||!manualChecks.some(c=>c.key===check)||!['yes','no','unknown'].includes(status))throw new Error('Məhsul yoxlaması düzgün deyil.');state.growth||={};state.growth.checks||={};state.growth.checks[pageId]||={};state.growth.checks[pageId][check]={status,updatedAt:new Date().toISOString()};}
...
export function saveOutreach(state,input){const source=(state.backlinks||[]).find(b=>b.id===input.backlinkId);const page=state.catalog.pages.find(p=>p.id===input.pageId&&p.url&&pageKey(p.url));if(!source||!page)throw new Error('Mənbə və məhsul səhifəsini seçin.');const fresh=outreachTemplate(source,page.url);const subject=input.subject??fresh.subject,body=input.body??fresh.body;if(typeof subject!=='string'||typeof body!=='string'||!subject.trim()||!body.trim()||subject.length>200||body.length>5000)throw new Error('Mövzu 1–200, mətn 1–5000 simvol olmalıdır.');state.growth||={};state.growth.outreach||={};state.growth.outreach[source.id]={backlinkId:source.id,pageId:page.id,target:page.url,subject:subject.trim(),body:body.trim(),updatedAt:new Date().toISOString()};}
export function changeEffects(state,changes){
 ...
 return changes.filter(c=>c.status==='applied'&&state.catalog.pages.some(p=>p.id===c.pageId)).slice(0,20).map(c=>{
  ...
  if(!key||!latest)return {...base,reason:'ABŞ üzrə gündəlik Google hesabatını gətirin.'};
  if(truncated)return {...base,status:'limited',reason:'Gündəlik hesabatın sətir limiti dolub; etibarlı müqayisə göstərilmir.'};
  for(let d=beforeStart;d<=afterEnd;d=plus(d,1))if(d!==changed&&!coverage.has(d))return {...base,reason:'Dəyişiklikdən əvvəl və sonra 7 tam günün məlumatı lazımdır.'};
  if(changes.some(other=>other.id!==c.id&&other.pageId===c.pageId&&other.status==='applied'&&pacificDate(other.createdAt)>=beforeStart&&pacificDate(other.createdAt)<=afterEnd))return {...base,status:'overlap',reason:'Bu pəncərədə səhifəyə başqa düzəliş də edilib; təsirlər ayrılmır.'};
  ...
  if(!beforeRows.length||!afterRows.length)return {...base,status:'limited',reason:'Dövrlərdən birində səhifə sətri yoxdur. Çatışmayan məlumat sıfır klik sayılmır.'};
  const before=metrics(beforeRows),after=metrics(afterRows);return {...base,status:'available',reason:'Google-un qaytardığı sətirlər üzrə müşahidədir. Dəyişiklik günü çıxarılıb; səbəb-nəticə sübutu deyil.',before,after,clickDifference:after.clicks-before.clicks};
 });
}
export function growthData(state,changes=[],now=new Date()){
 ...
 for(const [path,page] of byPath){const present=pageRows(current,path),prior=pageRows(previous,path);if(!present.length)continue;const m=metrics(present),before=prior.length?metrics(prior):null;const eligible=page.type!=='product'||!page.status||page.status==='ACTIVE';
  if(eligible&&m.impressions>=100&&m.position>=3&&m.position<=20&&m.ctr<.03)opportunities.push({pageId:page.id,pageTitle:page.title,url:page.url,...m,previous:before,reason:'100+ görünmə, 3–20 orta mövqe, 3%-dən aşağı CTR. Bunlar seçim filtrləridir.',action:'Başlıq və meta təsviri alıcının axtarışına uyğunlaşdır; təklifi təsdiqdən əvvəl yoxla.'});
 }
 ...
 for(const r of state.technical?.pages||[])if(r.technical?.noindex||[404,410].includes(r.status))task(`technical:${r.pageId}`,'Yayımlanmış səhifəni yoxla',`${r.title}: ${r.technical?.noindex?'noindex göstərişi var':`HTTP ${r.status}`}. Bunun məqsədli olub-olmadığını yoxla.`,r.pageId,'Texniki SEO',100);
 for(const o of opportunities)task(`ctr:${o.pageId}`,'Görünmələri klikə çevirmək üçün mətni yoxla',`${o.pageTitle}: ${o.impressions} görünmə, ${o.clicks} klik, ${(o.ctr*100).toFixed(1)}% CTR.`,o.pageId,'editor',80+Math.min(9,Math.log10(o.impressions)));
 if(current&&previous&&!current.truncated&&!previous.truncated)for(const [path,p] of byPath){const a=pageRows(current,path),b=pageRows(previous,path);if(a.length&&b.length){const before=metrics(b),after=metrics(a);if(before.clicks>=20&&after.clicks<=before.clicks*.7)task(`decline:${p.id}`,'Klik azalmasını araşdır',`${p.title}: ${before.clicks} → ${after.clicks} ABŞ kliki. Səbəb hələ müəyyən edilməyib.`,p.id,'Google nəticələri',90);}}
 for(const p of audit(pages,state.mappings))if((p.type!=='product'||!p.status||p.status==='ACTIVE')&&p.issues.some(i=>['title_missing','title_duplicate','meta_missing','body_missing'].includes(i.code)))task(`seo:${p.id}`,'Məhsulun SEO mətnini tamamla',`${p.title}: ${p.issues.filter(i=>['title_missing','title_duplicate','meta_missing','body_missing'].includes(i.code)).map(i=>i.label).join(', ')}.`,p.id,'editor',65);
 for(const p of products)if(p.checks.some(c=>!c.automatic&&c.status!=='yes'))task(`readiness:${p.pageId}`,'Alış üçün vacib məlumatları yoxla',`${p.pageTitle}: çatdırılma, qaytarma və mobil alış yoxlaması.`,p.pageId,'checklist',40);
 for(const b of state.backlinks||[])if(b.status==='mention')task(`outreach:${b.id}`,'Linksiz brend qeydi üçün mətn hazırla',b.url,null,'outreach',50);
 if(!current)task('setup:google','ABŞ üzrə Google məlumatını gətir','Google hesabını qoşub hesabatı yenilə; sıralama və kliklər təxmin edilməyəcək.',null,'Google nəticələri',95);
 if(!pages.length)task('setup:catalog','Məhsul kataloqunu gətir','Məhsul səviyyəsində tapşırıqlar üçün kataloqu yenilə.',null,'sync',99);
 ...
 return {week,current:reportSummary(current),previous:reportSummary(previous),syncedAt:state.growth?.syncedAt||null,opportunities:opportunities.slice(0,50),queries:queries.slice(0,100),queryCount:queries.length,queryLimited:Boolean(queryReport?.truncated),products,tasks:selected,completed,effects:changeEffects(state,changes),outreach:Object.values(state.growth?.outreach||{}).filter(d=>(state.backlinks||[]).some(b=>b.id===d.backlinkId)),warnings:[...(current?.truncated?['Səhifə hesabatı sətir limiti ilə məhdudlaşıb.']:[]),...(queryReport?.truncated?['Açar söz hesabatı sətir limiti ilə məhdudlaşıb.']:[]),...(current&&new Date(now).getTime()-Date.parse(current.importedAt)>3*86400000?['Google məlumatları 3 gündən köhnədir; yeniləyin.']:[])],allTaskIds:tasks.map(t=>t.id)};
}
export function setTaskDone(state,input){const data=growthData(state);if(!data.allTaskIds.includes(input.taskId))throw new Error('Tapşırıq artıq aktual deyil; paneli yenilə.');state.growth||={};state.growth.done||={};if(input.done==='true')state.growth.done[input.taskId]={week:data.week,at:new Date().toISOString()};else delete state.growth.done[input.taskId];}
