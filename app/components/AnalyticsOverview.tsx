...
import type {Dashboard,ActionInput} from '../lib/types';
...
export default function AnalyticsOverview({data,busy,onAction,onNavigate,onEdit}:{data:Dashboard;busy:boolean;onAction:(input:ActionInput)=>void;onNavigate:(tab:string)=>void;onEdit:(id:string)=>void}){
  ...
  const selected=selectSearchReports(data.googleReports||[],data.google?.property,country,'current',data.googleDays||28).total;
  ...
  return <div className="analytics-grid">
    ...
    <section className="analytics-card analytics-google"><h2>Google performansı</h2><div className="google-card-filters"><label className="google-country-filter"><span>Ülke</span><select aria-label="Google performansı ülkesi" value={country} onChange={e=>setCountry(e.target.value)}><option value="all">Tüm ülkeler</option><option value="usa">ABD</option></select></label><label className="google-country-filter"><span>Tarih aralığı</span><select aria-label="Google performansı tarih aralığı" value={String(data.googleDays||28)} disabled={busy||!data.google?.connected} onChange={e=>onAction({intent:'google-sync',days:e.target.value})}><option value="7">Son 7 gün</option><option value="28">Son 28 gün</option><option value="90">Son 3 ay</option></select></label></div>{report?<><div className="google-stat-row"><div><span>Tıklama</span><strong>{format(report.totals.clicks)}</strong></div><div><span>Gösterim</span><strong>{format(report.totals.impressions)}</strong></div><div><span>CTR</span><strong>%{format(report.totals.ctr*100)}</strong></div></div><p className="analytics-note">{report.startDate} — {report.endDate} · Search Console site toplamı{report.truncated?' · Rapor sınırlı':''}</p></>:<div className="google-connect-empty"><s-icon type="chart-line"/><div><strong>{data.google?.connected?'İlk raporunuzu getirin':'Google verilerini bağlayın'}</strong><p>Gerçek tıklama ve gösterimlerinizi burada takip edin.</p></div></div>}<button className="analytics-link" onClick={()=>onNavigate('Google sonuçları')}>{report?'Raporları aç':'Search Console bağlantısı'} <s-icon type="arrow-right"/></button></section>
