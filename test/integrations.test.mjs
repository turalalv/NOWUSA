...
test('OAuth tickets are one-use, browser-bound and replay-proof; tokens stay out of status',async t=>{
 const db=await fixture(t),link=await prepareGoogle(db,shop),ticket=new URL(link).searchParams.get('ticket');const start=await startGoogle(db,ticket);await assert.rejects(startGoogle(db,ticket));const state=new URL(start.url).searchParams.get('state');assert(new URL(start.url).searchParams.get('code_challenge'));await assert.rejects(finishGoogle(db,{state,code:'test',cookie:'nosweat_google_state=wrong'}),/brauzerdə/);
...
test('OAuth denial consumes state and creates no connection',async t=>{const db=await fixture(t),link=await prepareGoogle(db,shop),start=await startGoogle(db,new URL(link).searchParams.get('ticket'));await assert.rejects(finishGoogle(db,{state:new URL(start.url).searchParams.get('state'),error:'access_denied',cookie:start.cookie}),/icazəsi/);assert.equal(await db.googleConnection.count(),0);assert.equal(await db.googleOAuth.count(),0);});
...
test('compression previews save originals, reject stale sources and verify uploaded bytes',async t=>{
 ...
 current=Buffer.from('changed');await assert.rejects(applyCompression({db,shop,api,id:row.id,afterHash:row.afterHash,confirm:'APPLY',downloader:async()=>current}),/dəyişib/);assert.equal(writes,0);
...
test('USA refresh requests page, query and daily grains for both periods; partial failure preserves reports',async t=>{
 ...
 const factory=()=>({credentials:{refresh_token:'secret'},setCredentials(){},getAccessToken:async()=>{},request:async({data})=>{calls.push(data);if(fail&&calls.length===3)throw new Error('interrupted');assert.equal(data.dimensionFilterGroups[0].filters[0].expression,'usa');return {data:{rows:[{keys:data.dimensions.map(d=>d==='page'?'https://nosweatusa.com/products/example':d==='query'?'example':data.startDate),clicks:1,impressions:100,position:8}]}};}});
 const state={gsc:[{source:'old'}]};await syncGoogle(db,shop,state,factory);assert.equal(calls.length,6);assert.equal(state.growth.reports.length,4);assert.equal(state.growth.reports[0].rows[0].query,'example');assert.equal(state.growth.reports[1].rows[0].date,calls[2].startDate);assert.equal(state.gsc[0].source,'old');
